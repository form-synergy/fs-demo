...
<head>
    ...
    <style>
        ...
        body:not(.panel-active) #cards {
            margin-top: 10rem;
            opacity: 1;
            transition: opacity 0.2s, overflow 0.2s, height 0.2s cubic-bezier(.77, .59, .25, .6);
        }
    ...
    <?php
        ...
        /**
         * This example consists of all required steps
         *
...
<body>
    ...
    <!-- This file can be replaced. It demonstrate the implementation of data tags and the javaScript client -->
    <?php
         ...
         *
         * However, simple HTML with data attributes can do.
         */
    ...
    <!-- To create inline position to display an interaction as a modal -->
    <div 
    ...
    <script>
        // Instantiate the Form Synergy class with some other libraries
        ...
        FS.heartBeat({
            enabled: true,
            frequency: 10000 // Set heartbeat recurrence frequency in milliseconds.
        }).engage(); // To engage and start interaction.

        /**
         * Events: Available options are
         *  - open
         *  - hide
         *  - submit
         */
        FS.on('open', (el) => {
            setTimeout(function () {
                jQuery('#svg svg .active').not(el).removeClass('active');
        ...
        function remove_active(el) {
            if (jQuery(el).length < 1) {
                jQuery('#svg svg .active').removeClass('active');
                jQuery('body').removeClass('panel-active');
                remove_active(el);
            } else {
                setTimeout(function () {
                    remove_active(el);
         ...
         */
        jQuery(document).on('click', 'button[data-fs-etag]', function (e) {
            jQuery('body').addClass('panel-active');
        });

        jQuery(document).on('click', '[href="#cards"]', function (e) {
            jQuery('body').removeClass('panel-active');
